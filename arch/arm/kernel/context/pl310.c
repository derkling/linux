...
struct pl310_registers {
	...
	char paddinga[0x100];
	/* 0x900 */ volatile unsigned int lockdown[16];
	char paddingb[0x010];
...
void  clean_inv_range_pl310(void *start, unsigned size,
				       unsigned pl310_address)
{
	unsigned addr;
	struct pl310_registers *pl310 = (struct pl310_registers *)
						pl310_address;
	...
	while (addr <= size + (unsigned) start) {
		...
		/*
		 * For this to work on L220 we would have to poll the C
		 * bit now
		 */
	}
	dmb();
...
void  clean_range_pl310(void *start, unsigned size,
				   unsigned pl310_address)
{
	unsigned addr;
	struct pl310_registers *pl310 = (struct pl310_registers *)
	...
	while (addr <= size + (unsigned) start) {
		...
		addr += cache_line_size;
		/*
		 * For this to work on L220 we would have to poll the C bit
		 * now
		 */
	}
	dmb();
...
void  inv_range_pl310(void *start, unsigned size,
				 unsigned pl310_address)
{
	unsigned addr;
	struct pl310_registers *pl310 = (struct pl310_registers *)
...
void  save_pl310(u32 *pointer, unsigned pl310_address)
{
	struct pl310_registers *pl310 = (struct pl310_registers *)
						pl310_address;
	struct pl310_context *context = (struct pl310_context *) pointer;
	int i;

	/*
	 * TODO: are all these registers are present in earlier PL310
	 * versions ?
	 */
...
void  restore_pl310(u32 *pointer, unsigned pl310_address,
				int dormant)
{
	struct pl310_registers *pl310 = (struct pl310_registers *)
						pl310_address;
