...
DEFINE_STATIC_KEY_FALSE(_energy_model_ready);
 ...
 * to use such a model on very complex platform for some subsystems (such as
 * the scheduler) because of the associated overheads. The abritrary
 * constraint below makes it usable up to 16 CPUs with per-CPU DVFS.
 */
#define EM_MAX_COMPLEXITY 256

static void check_start(void)
{
	struct energy_model *em = rcu_dereference(energy_model);
	struct em_freq_domain *fd;
	int cpu, nb_fd = 0;

	/* Check that all possible CPUs are linked to a capacity state table */
	...
	for_each_freq_domain(fd, em)
		nb_fd++;
	if (nb_fd * num_possible_cpus() > EM_MAX_COMPLEXITY) {
		pr_warn("complexity is too high, aborting");
		return;
	}
...
int em_build_freq_domain(cpumask_t *span, int nr_states, long (*fun)(long*, int))
{
	...
	tmp->cap_states = kcalloc(nr_states, sizeof(*tmp->cap_states), GFP_KERNEL);
	if (!tmp->cap_states)
		goto free_tmp;
	...
	/* Enable energy_model_complete when all possible CPUs are covered */
	check_start();
