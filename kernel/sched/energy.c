...
static int build_energy_model(int cpu, cpumask_t *fdom)
{
	...
	for (i = 0, freq = 0; i < opp_cnt; i++, freq++) {
		...
		//dev_pm_opp_put(opp);
		if (!power) {
			pr_err("CPU%d: No power for OPP %d.\n", cpu, i+1);
			return -1;
		}
...
void start_sched_energy(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		cpu_dev = get_cpu_device(cpu);
		dev_pm_opp_of_get_sharing_cpus(cpu_dev, &(tmp->fdom));
		cpumask_set_cpu(cpu, &(tmp->fdom));
		list_add(&(tmp->next), &freq_domains);
		pr_info("Freq. domain: %*pbl\n", cpumask_pr_args(&(tmp->fdom)));
