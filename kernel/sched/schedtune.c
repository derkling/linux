...
int schedtune_accept_deltas(int nrg_delta, int cap_delta, struct task_struct *task) {
	...
	if (nrg_delta < 0 && cap_delta < 0) {
		/*
		 * energy_payoff criteria:
		 *    cap_delta / nrg_delta < perf_gain / cap_gain
		 * which is:
		 *    nrg_delta * perf_gain > perf_delta * cap_gain
		 */
		energy_payoff  = nrg_delta * threshold_gains[perf_constrain_idx].nrg_gain;
		energy_payoff -= cap_delta * threshold_gains[perf_constrain_idx].cap_gain;
