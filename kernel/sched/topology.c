...
static void free_sfd(struct sched_energy_fd *sfd)
{
	struct sched_energy_fd *tmp;
...
static struct sched_energy_fd *sfd_init(int cpu)
{
	struct em_freq_domain *fd = em_cpu_get(cpu);
						...
						struct sched_energy_fd *sfd)
{
	...
	printk(KERN_DEBUG "root_domain %*pbl: sfd:", cpumask_pr_args(cpu_map));

	while (sfd) {
		printk(KERN_CONT " { fd%d cpus=%*pbl nr_cstate=%d }",
				cpumask_first(freq_domain_span(sfd)),
				cpumask_pr_args(freq_domain_span(sfd)),
				em_fd_nr_cap_states(sfd->fd));
		sfd = sfd->next;
	}
...
static void build_freq_domains(const struct cpumask *cpu_map)
{
	...
	for_each_cpu(i, cpu_map) {
		...
		/* Create the new sfd and add it to the local list. */
		tmp = sfd_init(i);
		if (!tmp)
			goto free;
		tmp->next = sfd;
...
/*
 * This static_key is set if at least one root domain meets all conditions
 * for EAS to start. Those conditions are:
...
{
	/*
	 * The condition for EAS to start are checked during the root_domain
	 * creation. If a rd meets all conditions, it will have a non-null
...
match2:
	...
	for (i = 0; i < ndoms_new; i++) {
		for (j = 0; j < n; j++) {
			if (cpumask_equal(doms_new[i], doms_cur[j])
			    && cpu_rq(cpumask_first(doms_cur[j]))->rd->sfd)
			       goto match3;
		}
