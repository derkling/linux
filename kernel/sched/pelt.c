...
int pelt_update_task_util_blocked(struct task_struct *p,
				  struct cfs_rq *prev, struct cfs_rq *next)
{
	 ...
	 */
	if (!(p->pelt.attrs.last_update_time && prev))
		return;
	...
	if (pelt_update_sum(p_last_update_time, &p->pelt.attrs,
			    PELT_UTIL, 0, 0, 0) {
		pelt_update_avg(&p->pelt.attrs, 0, 0);
		trace_pelt_se_tp(se);
	}

	p->pelt.attrs.last_update_time = n_last_update_time;
}
...
accumulate_sum(u64 delta, struct sched_avg *sa,
	       unsigned long load, unsigned long runnable, int running,
	       unsigned int avg_period, unsigned int avg_max)
{
