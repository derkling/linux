...
TRACE_EVENT(cbs_burst,
	...
	TP_STRUCT__entry(
		...
		__field(	u32,	burst_time_sp	)
		__field(	u32,	burst_time	)
	...
	TP_fast_assign(
		...
		__entry->burst_time_sp	= cbs_se->burst_time_sp;
		__entry->burst_time	= cbs_se->burst_time;
		__entry->burst_interval	= cbs_se->burst_interval_ns;
...
TRACE_EVENT(cbs_round,
	...
	TP_STRUCT__entry(
		...
		__field( u64,		round_correction)
		__field( u64,		round_error	)
		__field( u64,		exec_runtime	)
	...
	TP_fast_assign(
		...
		__entry->round_correction	= cbs_rq->round_correction;
		__entry->round_error		= cbs_rq->round_error;
	...
	TP_printk("exec=%Lu | Rt=%Lu [%c] Re=%Lu ===> Nr=%u Rt_SP=%Lu [%c%c] Rt_corr=%Lu Rt_next=%Lu ",
		...
		__entry->round_correction,
		__entry->round_time_next)
);
