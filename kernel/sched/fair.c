							...
							int running)
{
	...
	u32 scaled_freq_runnable_contrib;
 ...
 * utilization is removed or add to the system (e.g. task wake-up). If both are
 * specified, the utilization is migrated.
 */
...
static int detach_tasks(struct lb_env *env)
{
	...
	if (!sched_ea(env) && env->imbalance <= 0)
		return 0;
...
static inline void update_sd_lb_stats(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	do {
		...
		if (sched_ea(env) && update_sd_pick_costliest(sds, sgs)) {
			sds->costliest = sg;
