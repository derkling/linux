...
compute_after:
	...
	if (sg->group_weight == 1 &&
		...
		eenv->before.group_util = group_util;
 ...
 * moving eenv.util_delta from eenv.src_cpu to eenv.dst_cpu.
 *
 * Note: compute_energy() may fail when racing with sched_domain updates.
 */
...
#define eenv_delta(__X)  ((int)eenv->after.__X - eenv->before.__X)
...
__update_perf_energy_deltas(struct energy_env *eenv)
{
	 ...
	 */
	eenv_before(speedup_idx) = (int)eenv_before(capacity) - task_util;
	eenv_after(speedup_idx)  = (int)eenv_after(capacity) - task_util;
	 ...
	 */
	eenv_before(delay_idx)  =  (int)SCHED_CAPACITY_SCALE;
	eenv_before(delay_idx) *= abs((int)eenv_before(group_util) - task_util);
	eenv_before(delay_idx) /=  eenv_before(group_util);
	if (eenv_before(group_util) < task_util)
		eenv_before(delay_idx) =  -eenv_before(delay_idx);
	eenv_after(delay_idx)   =  (int)SCHED_CAPACITY_SCALE;
	eenv_after(delay_idx)  *= abs((int)eenv_after(group_util) - task_util);
	eenv_after(delay_idx)  /=  eenv_after(group_util);
	if (eenv_after(group_util) < task_util)
		eenv_after(delay_idx) =  -eenv_after(delay_idx);
	/*
	 * Performance Index
	 *
	 *   PI := SPI - DLI
	 ...
	 * delay factors.
	 */
	eenv_before(perf_idx) = eenv_before(speedup_idx) - eenv_before(delay_idx);
	eenv_after(perf_idx) = eenv_after(speedup_idx) - eenv_after(delay_idx);
