...
static int should_we_balance(struct lb_env *env)
{
	...
	trace_printk("swb_tick: cpu=%i sd=%lx env=%lx\n",
		     env->dst_cpu, *cpumask_bits(sched_domain_span(env->sd)),
		     *cpumask_bits(env->cpus));
	...
	trace_printk("swb_tick: cpu=%i sd=%lx balance_mask=%lx\n",
		     env->dst_cpu, *cpumask_bits(sched_domain_span(env->sd)),
		     *cpumask_bits(group_balance_mask(sg)));
	...
	trace_printk("swb_tick: cpu=%i sd=%lx first_idle=%i\n",
		     env->dst_cpu, *cpumask_bits(sched_domain_span(env->sd)),
		     balance_cpu);

	if (balance_cpu == -1) {
		balance_cpu = group_balance_cpu(sg);
		trace_printk("swb_tick: cpu=%i sd=%lx balance=%i\n",
			     env->dst_cpu, *cpumask_bits(sched_domain_span(env->sd)),
			     balance_cpu);
	}
