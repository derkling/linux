			...
			do {
				...
				if (sg->group_weight == 1) {
					...
					if (eenv->usage_delta == 0 &&
						...
						eenv->cap.diff -= eenv->cap.before;
						trace_printk("gtk_cap_before: cpu=%d cap_idx=%d cap=%d diff=%d\n",
								eenv->src_cpu, cap_idx, eenv->cap.before, eenv->cap.diff);
					}
					/* Add capacity of dst CPU  (after task move) */
					if (eenv->usage_delta != 0 &&
						cpumask_test_cpu(eenv->dst_cpu, sched_group_cpus(sg))) {
						eenv->cap.after = sg->sge->cap_states[cap_idx].cap;
						eenv->cap.diff += eenv->cap.after;
						trace_printk("gtk_cap_after: cpu=%d cap_idx=%d cap=%d diff=%d\n",
								eenv->dst_cpu, cap_idx, eenv->cap.after, eenv->cap.diff);
					}
				}
...
static int energy_diff(struct energy_env *eenv)
{
	...
	do {
		if (eenv->src_cpu != -1 && cpumask_test_cpu(eenv->src_cpu,
			...
			eenv->cap.diff = eenv_before.cap.diff;
	...
	trace_sched_energy_diff(
			...
			eenv->cap.before, eenv->cap.after, eenv->cap.diff,
			eenv->energy_payoff);
...
out_affine:
	...
	if (sched_energy_freq()) {
		new_capacity = get_expected_capacity(new_cpu, p);
