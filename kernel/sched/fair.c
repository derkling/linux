		...
		unsigned long *max_util_bef, unsigned long *max_util_aft)
{
	...
	*max_util_aft = 0;
	...
	for_each_cpu(i, mask) {
		unsigned long cpu_util = get_cpu_usage(i);
...
static inline unsigned long __get_norm_cpu_usage(int cpu, int delta)
{
	unsigned long curr_capacity = get_curr_capacity(cpu);
...
{
	return __get_norm_cpu_usage(cpu, 0);
}

static inline int calc_usage_delta(struct energy_env *eenv, int cpu)
{
	if (unlikely(cpu == eenv->src_cpu))
		return -eenv->usage_delta;
	if (unlikely(cpu == eenv->dst_cpu))
		return eenv->usage_delta;
	return 0;
}

static unsigned get_group_max_usage(struct energy_env *eenv,
...
static unsigned get_group_max_norm_usage(struct energy_env *eenv,
						struct sched_group *sg)
{
	int i, delta;
	...
	for_each_cpu(i, sched_group_cpus(sg)) {
		delta = calc_usage_delta(eenv, i);
		max_usage = max(max_usage, __get_norm_cpu_usage(i, delta));
	}

	return max_usage;
}

static unsigned get_group_norm_usage(struct energy_env *eenv,
					struct sched_group *sg)
{
	int i, delta;
	...
	for_each_cpu(i, sched_group_cpus(sg)) {
		delta = calc_usage_delta(eenv, i);
		usage_sum += __get_norm_cpu_usage(i, delta);
	}
...
{
	int idx;
	unsigned long util = get_group_max_usage(eenv, eenv->sg_cap);
	...
	while (!cpumask_empty(&visit_cpus)) {
		...
		for_each_domain(cpu, sd) {
			...
			do {
				...
				group_util = get_group_norm_usage(eenv, sg);
...
static int energy_diff_util(int cpu, int util, int wakeups)
{
	...
	max_util_aft = get_cpu_usage(cpu) + util;
		...
		if (!sd->child) {
			...
			if (sd->flags & SD_SHARE_CAP_STATES) {
				...
				for_each_cpu(i, sched_domain_span(sd))
					sum_util += get_cpu_usage(i);
				aff_util_bef = sum_util;
			} else {
				aff_util_bef = get_cpu_usage(cpu);
			}
			...
			unused_util_bef = curr_state->cap
						- get_cpu_usage(cpu);
			unused_util_aft = new_state->cap - get_cpu_usage(cpu)
						- util;
		} else {
			/* Higher level */
...
static int energy_diff_task(int cpu, struct task_struct *p)
{
	...
	return energy_diff_util(cpu, p->se.avg.utilization_avg_contrib,
