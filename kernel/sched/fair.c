...
static long compute_energy(struct task_struct *p, int dst_cpu)
{
	...
	for_each_freq_domain(task_cpu(p), sfd) {
		max_util = sum_util = 0;
...
static int find_energy_efficient_cpu(struct task_struct *p, int prev_cpu)
{
	...
	for_each_freq_domain(prev_cpu, sfd) {
		...
		for_each_cpu_and(cpu, freq_domain_span(sfd), sched_domain_span(sd)) {
			...
			if (cpu == prev_cpu) {
				prev_energy = compute_energy(p, prev_cpu);
...
select_task_rq_fair(struct task_struct *p, int prev_cpu, int sd_flag, int wake_flags)
{
	...
	if (sd_flag & SD_BALANCE_WAKE) {
		record_wakee(p);
		want_energy = sched_energy_enabled(cpu_rq(cpu)->rd) &&
...
static struct sched_group *find_busiest_group(struct lb_env *env)
{
	...
	if (sched_energy_enabled(env->dst_rq->rd) &&
