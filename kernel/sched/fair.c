...
select_task_rq_fair(struct task_struct *p, int prev_cpu, int sd_flag, int wake_flags)
{
	...
	if (sd_flag & SD_BALANCE_WAKE) {
		...
		/*
		 * XXX: Forkees are not accepted in the energy-aware wake-up
		 * path because they don't have any useful utilization data yet
		 * and it's not possible to forecast their impact on energy
		...
		if (static_branch_unlikely(&sched_energy_present)) {
			...
			struct freq_domain *fd = rcu_dereference(rd->fd);
...
static struct sched_group *find_busiest_group(struct lb_env *env)
{
	...
	if (static_branch_unlikely(&sched_energy_present)) {
		...
		if (rcu_dereference(rd->fd) && !READ_ONCE(rd->overutilized))
			goto out_balanced;
	}
