				   ...
				   bool boosted, bool prefer_idle)
{
	...
	*backup_cpu = -1;
	 ...
	 * This will increase the chances to land on a reserved CPU for
	 * prefer_idle and boosted tasks while not impacting the selection
	 * of a CPU in the other cases.
	 */
	...
	do {
		for_each_cpu_and(i, tsk_cpus_allowed(p), sched_group_cpus(sg)) {
			...
			if (prefer_idle) {
				 ...
				 * - for !boosted tasks: the most energy efficient
				 *   CPU (i.e. smallest capacity_orig)
				 */
				if (idle_cpu(i)) {
					if (boosted &&
