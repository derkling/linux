 ...
 * Model) to execute a task are those for which we'll request a low OPP. When
 * there are several CPUs for which the OPP request will be the same, we don't
 * have enough data to break the tie beween them, because the Energy Model
 * only includes active power costs. With this model, if we assume that OPP
 * requests follow utilization (e.g. using schedutil), the CPU with the
 * maximum spare capacity in a frequency domain is guaranteed to be among the
 * best candidates of the frequency domain.
 *
