		  ...
		  int this_cpu, int sd_flag)
{
	 ...
	 * UTIL_AVG), so we allow for some task stuffing by using
	 * spare_capacity > task_util(p, UTIL_AVG)/2.
	 */
	if (this_spare > task_util(p, UTIL_AVG) / 2 &&
	    imbalance*this_spare > 100*most_spare)
		return NULL;
	else if (most_spare > task_util(p, UTIL_AVG) / 2)
...
static int cpu_util_wake(int cpu, struct task_struct *p)
{
	...
	if (cpu != task_cpu(p) || !p->se.avg.last_update_time)
		return cpu_util(cpu, UTIL_AVG);

	capacity = capacity_orig_of(cpu);
	util = max_t(long, cpu_rq(cpu)->cfs.avg.util_avg - task_util(p, UTIL_AVG), 0);
...
static int wake_cap(struct task_struct *p, int cpu, int prev_cpu)
{
	...
	return min_cap * 1024 < task_util(p, UTIL_AVG) * capacity_margin;
}
...
static int select_energy_cpu_brute(struct task_struct *p, int prev_cpu)
{
	...
	for_each_cpu_and(i, tsk_cpus_allowed(p), sched_domain_span(sd)) {
		...
		if (spare * 1024 < capacity_margin * task_util(p, UTIL_AVG))
			continue;
