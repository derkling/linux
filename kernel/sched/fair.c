	...
	do {
		for_each_cpu_and(i, &p->cpus_allowed, sched_group_span(sg)) {
			...
			if (prefer_idle) {
				...
				if (printk_enabled) {
					trace_printk("cpu=%d idle=%d capacity_orig=%lu capacity_curr=%lu wake_util=%lu new_util=%lu "
						     "spare_cap=%lu target_capacity=%lu target_max_spare_cap=%lu",
...
target:
	...
	trace_sched_find_best_target(p, prefer_idle, boosted, min_util,
