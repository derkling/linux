	...
	*backup_cpu = -1;
	...
	do {
		for_each_cpu_and(i, &p->cpus_allowed, sched_group_span(sg)) {
			 ...
			 * enqueued here. This value is used by the code below
			 * to support a task spreading strategy.
			 */
...
/*
 * group_smaller_max_cpu_capacity: Returns true if largest CPU in sched_group
 * sg has smaller per-CPU capacity than largest in sched_group ref.
 */
static inline bool
...
static inline void update_sg_lb_stats(struct lb_env *env,
			...
			int *overload, bool *overutilized, bool *misfit_task)
{
	unsigned long load;
	int i, nr_running;
