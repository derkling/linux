...
/* Scan all online CPUS to find the one that minimizes the system energy */
...
{
	struct sched_domain *tmp, *affine_sd = NULL, *energy_sd = NULL, *sd = NULL;
	...
	for_each_domain(cpu, tmp) {
		 ...
		 * non-overutilized sched_domain spanning over cpu and
		 * prev_cpu.
		 */
		...
		if (tmp->flags & sd_flag)
			sd = tmp;
		else if (!want_affine) // XXX: && !want_energy ???
			break;
	}
...
next_group:
	...
	if (env->sd->shared->overutilized != overutilized)
		env->sd->shared->overutilized = overutilized;
	...
	if (env->sd->parent &&
		...
		env->sd->parent->shared->overutilized = true;
}
