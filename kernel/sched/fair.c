...
static inline void update_overutilized_status(struct rq *rq)
{
	if (!READ_ONCE(rq->rd->overutilized) && cpu_overutilized(rq->cpu))
		WRITE_ONCE(rq->rd->overutilized, SG_OVERUTIL);
}
#else
static inline void update_overutilized_status(struct rq *rq) { }
#endif
...
/*
 * Estimates the energy that would be consumed if @p was migrated to @dst_cpu.
 * compute_energy() predicts what will be the utilization landscape of the CPUs
 * after the task migration, and uses the Energy Model to compute what would be
 * the energy if we decided to actually migrate that task.
 */
							...
							struct freq_domain *fd)
{
	...
	while (fd) {
		 ...
		 *
		 * If an entire frequency domain is out the current rd, it will
		 * not appear in its fd list and will not be accounted by
		 * compute_energy().
		 */
...
next_group:
	...
	if (!env->sd->parent) {
		...
		WRITE_ONCE(rd->overutilized, sg_status & SG_OVERUTIL);
	} else if (sg_status & SG_OVERUTIL) {
		WRITE_ONCE(env->dst_rq->rd->overutilized, SG_OVERUTIL);
	}
}
