...
static bool sugov_should_update_freq(struct sugov_policy *sg_policy, u64 time)
{
	...
	if (use_pelt() && sg_policy->work_in_progress)
		return false;
...
static void
sugov_get_util(unsigned long *util, unsigned long *max, u64 time, int cpu)
{
...
static unsigned int sugov_next_freq_shared(struct sugov_cpu *sg_cpu, u64 time)
{
	...
	unsigned int j;
	unsigned int ignore_cpu_thresh = TICK_NSEC;

	if (!use_pelt())
		ignore_cpu_thresh = walt_ravg_window;
				...
				unsigned int flags)
{
	...
	trace_sugov_util_update(sg_cpu->cpu, sg_cpu->util, max, flags);
