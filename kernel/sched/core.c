 ...
 * map allow to track how many TG's use the same clamp value and it defines
 ...
 * - rows maps the different clamp indexes
 *   i.e. minimum or maximum utilization
 * - columns maps the different clamp groups
 ...
 * Thus we allocate one more slot than what defined by CONFIG_UCLAMP_GROUPS.
 */
struct uclamp_map {
	int value;
	int tg_count;
...
/**
 * uclamp_group_free: check if a per-CPU clamp group is available
 ...
 * value.
 *
 * This function initialize a clamp group to track tasks from the fast-path.
 */
 ...
 *
 * When tasks are enqueue/dequeud from a CPU, the set of currently active
 * clamp groups is subject to change. Since each clamp group enforce a
 ...
 * groups have to be update for each CPU containing a RUNNABLE task belonging
 ...
 * released when there are not more task groups referencing its clamp value.
 */
 ...
 * where they are enqueued for execution.
 *
				   ...
				   unsigned int clamp_value)
{
	...
	while ((p = css_task_iter_next(&it)))
		uclamp_update_active_tasks(p, clamp_id, next_group_id);
...
static inline void uclamp_update_task(struct rq *rq, struct task_struct *p)
{
...

	{
		...
		bool get = uclamp_task_affect(p, UCLAMP_MIN);
 ...
 *
 * An empty task group can be removed only when it has not more tasks or child
 * groups. This means that we can also safely release all the reference
 * counting to clamp groups it was using.
 */
