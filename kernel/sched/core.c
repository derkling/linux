 ...
 *
 * A clamp group is not free if there is at least one SE's using a clamp value
 * mapped on the specified clamp_id. These SE's are reference counted by the
...
static inline void uclamp_cpu_update(struct rq *rq, int clamp_id)
{
	...
	/*
	 * Just for the UCLAMP_MAX value, in case there are not RUNNABLE
	 * task, we keep the CPU clamped to the value of the last task
	 * executed.
	...
	if (clamp_id == UCLAMP_MAX && max_value == UCLAMP_NONE) {
		struct task_struct *idle = idle_task(cpu_of(rq));
 ...
 *
 * Task groups also can track clamp value, which are used to constraint task
 * specific clamp values. When a task group change its clamp value, all the
 * (active) tasks belonging to that group must be update to ensure they are
 * refcounting the correct CPU's clamp value.
 *
				   ...
				   unsigned int clamp_value)
{
	...
	/* Update CPU's clamp group refcounts of active tasks */
	if (p)
...
#ifdef CONFIG_UCLAMP_TASK_GROUP
	/*
	 * CGroup enforced clamps have higher priority then
	 * task specific clamps.
	 */
