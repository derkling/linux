...
static void uclamp_group_inc(struct task_struct *p, struct uclamp_se *uc_se,
			     unsigned int clamp_id, unsigned int clamp_value)
{
	...
	if (uc_se->mapped)
		uclamp_group_dec(clamp_id, prev_group_id);
			 ...
			 loff_t *ppos)
{
	...
	if (old_min != sysctl_sched_uclamp_util_min) {
		uclamp_group_inc(NULL, &uclamp_default[UCLAMP_MIN],
				 UCLAMP_MIN, sysctl_sched_uclamp_util_min);
	}
	if (old_max != sysctl_sched_uclamp_util_max) {
		uclamp_group_inc(NULL, &uclamp_default[UCLAMP_MAX],
				 ...
				 const struct sched_attr *attr)
{
	...
	if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MIN) {
		...
		uclamp_group_inc(p, &p->uclamp[UCLAMP_MIN],
				 UCLAMP_MIN, lower_bound);
	}
	if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MAX) {
		p->uclamp[UCLAMP_MAX].user_defined = true;
		uclamp_group_inc(p, &p->uclamp[UCLAMP_MAX],
...
void uclamp_exit_task(struct task_struct *p)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		if (!p->uclamp[clamp_id].mapped)
			continue;
		uclamp_group_dec(clamp_id, p->uclamp[clamp_id].group_id);
	}
}
...
static void uclamp_fork(struct task_struct *p, bool reset)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		...
		p->uclamp[clamp_id].active = false;
		uclamp_group_inc(NULL, &p->uclamp[clamp_id],
...
static void __init init_uclamp(void)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		...
		uclamp_group_inc(NULL, uc_se, clamp_id, uclamp_none(clamp_id));
		...
		uclamp_group_inc(NULL, uc_se, clamp_id, uclamp_none(UCLAMP_MAX));
...
static inline void free_uclamp_sched_group(struct task_group *tg)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id)
		uclamp_group_dec(clamp_id, tg->uclamp[clamp_id].group_id);
}
					   ...
					   struct task_group *parent)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		uclamp_group_inc(NULL, &tg->uclamp[clamp_id], clamp_id,
				  ...
				  struct cftype *cftype, u64 min_value)
{
	...
	uclamp_group_inc(NULL, &tg->uclamp[UCLAMP_MIN], UCLAMP_MIN, min_value);
				  ...
				  struct cftype *cftype, u64 max_value)
{
	...
	/* Update TG's reference count */
	uclamp_group_inc(NULL, &tg->uclamp[UCLAMP_MAX], UCLAMP_MAX, max_value);
