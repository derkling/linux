 ...
 * expected to be always valid. If we detect they are not, as defensive
 * programming, enforce the expected state and warn.
 */
...
static inline void uclamp_rq_inc(struct rq *rq, struct task_struct *p)
{
	...
	/* Reset clamp idle holding when we have one RUNNABLE task */
	if (rq->uclamp_flags & UCLAMP_FLAG_IDLE)
		rq->uclamp_flags &= ~UCLAMP_FLAG_IDLE;
}
...
static void __init init_uclamp(void)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		...
		uc_se = &init_task.uclamp_req[clamp_id];
...
#ifdef CONFIG_UCLAMP_TASK_GROUP
		uc_se = &root_task_group.uclamp_req[clamp_id];
		uc_se->bucket_id = bucket_id;
		uc_se->value = value;
					   ...
					   struct task_group *parent)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		tg->uclamp_req[clamp_id].value =
			parent->uclamp_req[clamp_id].value;
		tg->uclamp_req[clamp_id].bucket_id =
			parent->uclamp_req[clamp_id].bucket_id;
				...
				unsigned int clamp_id)
{
	...
	css_for_each_descendant_pre(css, top_css) {
		...
		printk("Updated %s: req(value=%u, bucket_id=%u), "
		       "eff(value=%u, bucket_id=%u)\n",
		       path,
		       css_tg(css)->uclamp_req[clamp_id].value,
		       css_tg(css)->uclamp_req[clamp_id].bucket_id,
