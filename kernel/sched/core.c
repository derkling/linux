...
/**
 * uclamp_cpus: tracks clamp values based on CPU's active tasks
 *
 ...
 * user-space process (slow-path) but it requires a synchronization with
 ...
 * map allow to track which TG's uses a clamp value thus definition the clamp
 * group index to use for the per-CPU accounting on the fast-path.
 *
 * Since we support utilization clamping using both a minimum and maximum
 * value, a matrix is used to map clamp values, here:
 * - rows maps the different clamping indexes
 *   i.e. minimum or maximum utilization)
 * - colums maps the different clamping groups
 *   i.e. TG's with similar clamp value for a given clamp index
 *
 * NOTE: clamp group 0 is reserved for accouting non clamped tasks. Thus we
 *       alloacate one more slot with respect to what user configured via
 *       the CONFIG_UCLAMP_GROUPS Kconfig option.
 */
struct uclamp_map {
	int 			value;
	int 			tg_count;
	struct list_head 	tg_list;
	raw_spinlock_t		tg_list_lock;
};
static struct uclamp_map uclamp_maps[UCLAMP_CNT][CONFIG_UCLAMP_GROUPS+1];

/**
 * uclamp_group_free: check if a per_CPU clamp group is available
 ...
 * mapped on the specified clamp_id. These TG's are tracked by the tg_list of
 * a group.
 *
 * Return: true if there are not TG's mapped on the specified clamp
 *         index and group
 */
static inline bool uclamp_group_free(int clamp_id, int group_id)
{
	struct uclamp_map *uc_map = &uclamp_maps[clamp_id][0];

	return (list_empty(&uc_map[group_id].tg_list));
}
 ...
 * @clamp_value: the utilization clamping value to map
 *
 * Each different clamp value, for a given clamp index (i.e. min/max utilization
 * clamping), is mapped by a clamp group which index is use by the fast-path
 * code to keep track of active tasks requiring a certain clamp value.
 *
 * This function initialize a clamp group to properly track tasks from the
 * fast-path.
 */
				     ...
				     unsigned int clamp_value)
{
	...
	for_each_possible_cpu(cpu) {
		cpu_cc = &per_cpu(uclamp_cpus[clamp_id], cpu);
		cpu_cc->group[group_id].value = clamp_value;
		cpu_cc->group[group_id].tasks = 0;
		cpu_cc->group[group_id].eq_count = 0;
		cpu_cc->group[group_id].dq_count = 0;
		cpu_cc->value = uclamp_none(clamp_id);
	}
}

/**
 * uclamp_group_reset: release a specified clamp group
 * @clamp_id: the utilization clamp index (i.e. min or max clamping)
 * @group_id: the group index to release
 *
 * A clamp group can be release every time there are not more TGs using the
 ...
 * its index in the corresponding per_cpu(struct uclamp_group) variable.
 *
 * Since only a limited number of utilization clamp groups are allowed,
 * if no groups have been assinged for the specified value, a new group is
 * assigned if possible. Otherwise an error is returned, meaning that a
 * different clamp value is not supported.
 */
static int
uclamp_group_find(int clamp_id, unsigned int clamp_value)
{
	...
	if (group_id > CONFIG_UCLAMP_GROUPS)
...
/**
 * uclamp_update_cpu: update the utilization clamp of a CPU
 * @cpu:      the CPU which utilization clamp has to be updated
 * @clamp_id: the utilization clamp to update
 *
 * Once tasks are enqueue/dequeud from a CPU, the set of currently active
 * clamp groups is subject to change. Since each clamp group enfoce a
 * different utilizaton clamp value, once the set of these groups change it
 * can be required to re-compute what is the maximum clamp value to apply for
 * that CPU.
 *
 * For the specified utilization clamp, this method computes the new CPU
 * utilization clamp to use until the next change on the set of tasks active
 * on that CPU.
 */
static inline void uclamp_update_cpu(int cpu, int clamp_id)
{
	struct uclamp_cpu *cpu_cc = &per_cpu(uclamp_cpus[clamp_id], cpu);
	unsigned int max_value;
	unsigned int group_id;

	max_value = 0;
	for (group_id = 0; group_id <= CONFIG_UCLAMP_GROUPS; ++group_id) {

		/* Ignore inactive clamp groups, i.e. no RUNNABLE tasks */
		if (!uclamp_group_active(cpu_cc, group_id))
			continue;

		/* Both min and max clamp are MAX aggregated */
		max_value = max(max_value,
				cpu_cc->group[group_id].value);

		/* Stop if we reach the max possible clamp */
		if (max_value >= SCHED_CAPACITY_SCALE)
			break;
	}
	cpu_cc->value = max_value;
}

/**
 * uclamp_cpu_get(): increase reference count for a clamp group on a CPU
 * @p:        the task being enqueued on a CPU
 * @cpu:      the CPU where the clamp group has to be referenced
 * @clamp_id: the utilization clamp (e.g. min or max utilization) to reference
 *
 * Once a task is activated on a CPU, the clamp group currently defined by the
 * task's TG::uclamp.group_id is reference counted on that CPU.
...
{
	unsigned int clamp_value = task_group(p)->uclamp[clamp_id].value;
	struct uclamp_cpu *cpu_cc = &per_cpu(uclamp_cpus[clamp_id], cpu);
	int group_id;
	...
	cpu_cc->group[group_id].eq_count += 1;
	 ...
	 */
	if (cpu_cc->value < clamp_value)
		cpu_cc->value = clamp_value;

	trace_printk("cpu_get: pid=%d comm=%s cpu=%d clamp_id=%d clamp_cpu=%d "
		     "group_id=%d clamp_value=%d tasks=%d eqc=%d dqc=%d",
			p->pid, p->comm, cpu, clamp_id, cpu_cc->value, group_id, clamp_value,
			cpu_cc->group[group_id].tasks,
			cpu_cc->group[group_id].eq_count,
			cpu_cc->group[group_id].dq_count);
 ...
 * @cpu:      the CPU from where the clamp group can be released
 * @clamp_id: the utilization clamp (e.g. min or max utilization) to release
 *
 * Once a task is removed from a CPU, the clamp group reference counted by the
 * task's task_struct::uclamp_group_id is decrease for that CPU.
 */
static inline void uclamp_cpu_put(struct task_struct *p, int cpu, int clamp_id)
{
	...
	cpu_cc->group[group_id].tasks -= 1;
	cpu_cc->group[group_id].dq_count += 1;
	...
	if (cpu_cc->group[group_id].tasks > 0) {
		clamp_value = cpu_cc->group[group_id].value;
		goto done;
	}
	...
	trace_printk("cpu_put: pid=%d comm=%s cpu=%d clamp_id=%d clamp_cpu=%d "
			...
			cpu_cc->value, group_id, clamp_value,
			cpu_cc->group[group_id].tasks,
			cpu_cc->group[group_id].eq_count,
			cpu_cc->group[group_id].dq_count);
...
/**
 * uclamp_update_active_tasks: update the clamp group of a runnable tasks
 * @p:        a task to update
 ...
 * groups have to be update for each CPU containing a RUNNABLE tasks belonging
 * to this tasks group. Non RUNNABLE tasks are not update since they will be
...
uclamp_update_active_tasks(struct task_struct *p, int clamp_id)
{
	 ...
	 * This is the same locking schema use by __set_cpus_allowed_ptr().
	 */
	 ...
	 * enqueue with the proper clamp group value.
	 */
 ...
 * counting for the clamp group mapping its current clamp value. This allows
 * to release a clamp group when there are not more task groups referencing
 * its clamp value.
 */
static inline void uclamp_group_put(int clamp_id, struct uclamp_tg *uc_tg)
{
	struct uclamp_map *uc_map = &uclamp_maps[clamp_id][0];
	int group_id = uc_tg->group_id;
	...
	raw_spin_lock(&uc_map[group_id].tg_list_lock);
	list_del(&uc_tg->tg_node);
	INIT_LIST_HEAD(&uc_tg->tg_node);
	uc_map[group_id].tg_count -= 1;
	raw_spin_unlock(&uc_map[group_id].tg_list_lock);
...
/**
 * uclamp_group_get: increase reference count for a clamp group
 ...
 *
 * Each time a task group change the utilization clamp value, for a specified
 * clamp index, we need to find (if available) a clamp group which can be used
 * to track this clamp value. The corresponding clamp group index will be used
 * by the task in this task group to reference count the clamp value on the
 * CPU where they are enqueued for execution.
 *
...
{
	struct uclamp_map *uc_map = &uclamp_maps[clamp_id][0];
	int group_id = UCLAMP_NONE;
	struct css_task_iter it;
	struct task_struct *p;

	/* Lookup for a usable utilization clamp group */
	group_id = uclamp_group_find(clamp_id, clamp_value);
	if (group_id < 0) {
		pr_err("Cannot allocate more than %d utilization clamp groups\n",
		       CONFIG_UCLAMP_GROUPS);
		return -ENOSPC;
	}

	/* Allocate new clamp group for this clamp value */
	if (uclamp_group_free(clamp_id, group_id))
		uclamp_group_init(clamp_id, group_id, clamp_value);

	/* Detach the TG from previous clamp group */
	uclamp_group_put(clamp_id, uc_tg);

	/* Update TG's clamp values */
	uc_tg->group_id = group_id;
	uc_tg->value = clamp_value;

	raw_spin_lock(&uc_map[group_id].tg_list_lock);
	list_add(&uc_tg->tg_node, &uc_map[group_id].tg_list);
	uc_map[group_id].tg_count += 1;
	raw_spin_unlock(&uc_map[group_id].tg_list_lock);
...
/**
 * uclamp_update_task: update clamp group referenced by a task
 ...
 * requirements, each active task reference count a clamp group in the CPU
 * they are currently enqueud for execution.
 *
 ...
 * calling into the scheduling class calls, which will eventually update
 * schedutil considering this task requirements.
 */
static inline void uclamp_update_task(struct rq *rq, struct task_struct *p)
{
	int cpu = cpu_of(rq);
	int clamp_id;
 ...
 *
 * A newly created task group inherits the utilization clamping values for all
 * clamp index from its parent task group.
 * This ensure that its values are properly initialized and that the task
...
/**
 * release_uclamp_sched_group: release a TG's for utilization clamping
 * @tg: the task group to release
 *
 * An empty task group can be release only when it has not more tasks on child
 * groups. This means that we can also safely release the reference counting
 * for the clamp group it was using.
 */
...
static inline void init_uclamp(void)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		...
		group_id = 0;
		for ( ; group_id <= CONFIG_UCLAMP_GROUPS; ++group_id) {
			uc_map[group_id].value = uclamp_none(clamp_id);
			INIT_LIST_HEAD(&uc_map[group_id].tg_list);
			raw_spin_lock_init(&uc_map[group_id].tg_list_lock);
		}
		/* Init CPU's clamp groups */
		for_each_possible_cpu(cpu) {
			cpu_cc = &per_cpu(uclamp_cpus[clamp_id], cpu);
			memset(cpu_cc, UCLAMP_NONE, sizeof(struct uclamp_cpu));
		}
	}
...
void printk_cpu_report(void)
{
	...
	for ( ; clamp_id < UCLAMP_CNT; ++clamp_id) {
		...
		while (group_id <= CONFIG_UCLAMP_GROUPS) {
			if (uclamp_group_free(clamp_id, group_id)) {
				...
				pos += sprintf(pos, "%4d(%2d) ", group_id,
					       uc_map[group_id].tg_count);
			}
			++group_id;
		}
	...
	for_each_possible_cpu(cpu) {
		...
		for ( ; clamp_id < UCLAMP_CNT; ++clamp_id) {
			cpu_cc = &per_cpu(uclamp_cpus[clamp_id], cpu);
			pos += sprintf(pos, "| clamp_id=%d(%4d): ", clamp_id, cpu_cc->value);

			group_id = 0;
			while (group_id <= CONFIG_UCLAMP_GROUPS) {
				if (cpu_cc->group[group_id].value == UCLAMP_NONE) {
					pos += sprintf(pos, "----(--) ");
				} else {
					pos += sprintf(pos, "%4d(%2d) ",
						cpu_cc->group[group_id].value,
						cpu_cc->group[group_id].tasks);
				}
				++group_id;
				      ...
				      struct cftype *cftype, u64 min_value)
{
	...
	/* Ensure to not exceed the maximum utilization allowed */
	if (tg->uclamp[UCLAMP_MAX].value < min_value)
		goto out;

	/* Ensure min clamp fits within parent clamp */
	if (tg->parent &&
	    tg->parent->uclamp[UCLAMP_MIN].value > min_value)
		goto out;

	/* Ensure each child is a subset of this TG */
				      ...
				      struct cftype *cftype, u64 max_value)
{
	...
	/* Ensure to not go below the minimum utilization required */
	if (tg->uclamp[UCLAMP_MIN].value > max_value)
		goto out;

	/* Ensure max clamp fits within parent clamp */
	if (tg->parent &&
				     ...
				     struct cftype *cft)
{
	...
	rcu_read_lock();
	tg = css_tg(css);
	util_min = tg->uclamp[UCLAMP_MIN].value;
	rcu_read_unlock();
				     ...
				     struct cftype *cft)
{
	...
	rcu_read_lock();
	tg = css_tg(css);
	util_max = tg->uclamp[UCLAMP_MAX].value;
	rcu_read_unlock();
