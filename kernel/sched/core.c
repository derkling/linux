 ...
 * - the task specific clamp value, explicitely requested from userspace
 * - the task group effective clamp value, for tasks not in the root group or
 *   in an autogroup
 * - the system default clamp value, defined by the sysadmi
						     ...
						     unsigned int clamp_id)
{
	...
	/* Keep the compiler quiet on !CONFIG_UCLAMP_TASK_GROUP */
	if (false)
		goto done;
...
#define UCLAMP_GRPERR "invalid SE clamp group [%u:%u] refcount\n"
	...
	if (res != uc_map_old.data) {
		printk("    uclamp_group_put: clamp_id=%u group_id=%u count=%lu FAILED\n",
...
#define UCLAMP_MAPERR "clamp value [%u] mapping to clamp group failed\n"
	...
	res = atomic_long_cmpxchg(&uc_maps[group_id].adata,
				  uc_map_old.data, uc_map_new.data);
	if (res != uc_map_old.data) {
		printk("    uclamp_group_get: clamp_id=%u group_id=%u count=%lu FAILED\n",
		       clamp_id, group_id, (unsigned long)uc_map_old.se_count);
		goto retry;
	}
...
void uclamp_exit_task(struct task_struct *p)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		if (!p->uclamp[clamp_id].mapped)
			continue;
		uclamp_group_put(clamp_id, p->uclamp[clamp_id].group_id, "SeExt");
	}
}
