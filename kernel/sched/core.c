...
cap_clamp_update_capacity(struct task_struct *p, unsigned int cap_idx)
{
	 ...
	 *
	 * This risks to lock the (previous) RQ's of a !RUNNABLE task but it's
	 * required to serialize the capacity_{min,max} updates with enqueues,
	 * dequeues and migration operations, which is the same locking schema
	 * already in use by __set_cpus_allowed_ptr().
	 */
				      ...
				      struct cftype *cftype, u64 value)
{
	...
	if (tg->parent->cap_clamp[CAP_CLAMP_MIN] > min_value)
		goto out;
				      ...
				      struct cftype *cftype, u64 value)
{
	...
	if (tg->parent->cap_clamp[CAP_CLAMP_MAX] < max_value)
		goto out;
