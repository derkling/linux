...
extern unsigned int uclamp_cpu(struct rq *rq, unsigned int util);
extern unsigned int uclamp_task(struct task_struct *p);
 ...
 */
static __always_inline unsigned long __cpu_util(int cpu,
		unsigned long util_cfs, unsigned long util_rt,
		unsigned long util_dl, unsigned long util_irq)
{
	 ...
	 */
	freq_selection = (util_cfs & util_rt & util_dl & util_irq) == ~0;
	 ...
	 *
	 * For requency selection, CFS utilization can be boosted or capped,
	 * depending on utilization clamp constraints configured for currently
	 * RUNNABLE tasks.
	 */
	if (!~util_cfs)
	...
	if (freq_selection && util_cpu)
		util_cpu = uclamp_cpu(cpu, util_cpu);
#endif
	if (util_cpu >= max)
		return max;
	 ...
	 *                max
	 */
	util_cpu  = scale_irq_capacity(util_cpu, util_irq, max);
	util_cpu += util_irq;
	if (util_cpu >= max)
		return max;
