 ...
 *
 * Keep track of tasks active on a CPUs to aggregate their clamp values.  A
 ...
 * @p: the task to check
 *
 * A task affects the utilization clamp of a CPU if it reference a
 * valid clamp group index for at leas one clamp index.
 *
 * Return: true if p is currenlty clamping the utilization of its CPU.
 */
static inline bool uclamp_task_active(struct task_struct *p)
{
	...
	for (clamp_id = 0; clamp_id < UCLAMP_CNT; ++clamp_id) {
		if (uclamp_task_affect(p, clamp_id))
			return true;
	}
 ...
 *
 * The utilization clamp value for a CPU depends on the set of tasks currently
 * active on a CPU and their specific util_{min,max} constraints.
 * A max aggregated value is tracked for each CPU and returned and returned by
 * this function. An IDLE CPU never enforce a clamp value.
 *
 ...
 * [0..SCHED_CAPACITY_SCALE] range, this function returns a clamped of this
 * signal considering the current clamp values for the specified CPU.
 *
