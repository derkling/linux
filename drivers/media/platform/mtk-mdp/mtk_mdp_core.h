...
#define mtk_mdp_m2m_active(dev)	test_bit(ST_M2M_RUN, &(dev)->state)
#define mtk_mdp_m2m_pending(dev) test_bit(ST_M2M_PEND, &(dev)->state)
#define mtk_mdp_m2m_opened(dev)	test_bit(ST_M2M_OPEN, &(dev)->state)
...
/**
 * struct mtk_mdp_m2m_device - v4l2 memory-to-memory device data
 * @vfd: the video device node for v4l2 m2m mode
 * @m2m_dev: v4l2 memory-to-memory device data
 * @ctx: hardware context data
 * @refcnt: the reference counter
 */
struct mtk_mdp_m2m_device {
...
const struct mtk_mdp_fmt *mtk_mdp_find_fmt(u32 *pixelformat, u32 *mbus_code,
					   u32 index);
...
int mtk_mdp_process_done(void *priv);
