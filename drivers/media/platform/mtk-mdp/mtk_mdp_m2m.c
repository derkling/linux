...
static int mtk_mdp_ctx_stop_req(struct mtk_mdp_ctx *ctx)
{
	...
	curr_ctx = v4l2_m2m_get_curr_priv(mdp->m2m.m2m_dev);
	if (!mtk_mdp_m2m_pending(mdp) || (curr_ctx != ctx))
		return 0;
...
static void __mtk_mdp_m2m_job_abort(struct mtk_mdp_ctx *ctx)
{
	int ret;
...
void mtk_mdp_m2m_job_finish(struct mtk_mdp_ctx *ctx, int vb_state)
{
	...
	if (src_vb && dst_vb) {
		...
		v4l2_m2m_job_finish(ctx->mdp_dev->m2m.m2m_dev,
				    ctx->m2m_ctx);
	}
}
...
{
	__mtk_mdp_m2m_job_abort((struct mtk_mdp_ctx *)priv);
}
				    ...
				    struct v4l2_format *f)
{
	...
	pix = &f->fmt.pix_mp;
	frame->fmt = mtk_mdp_find_fmt(&pix->pixelformat, NULL, 0);
...
static int mtk_mdp_m2m_open(struct file *file)
{
	...
	ctx->m2m_ctx = v4l2_m2m_ctx_init(mdp->m2m.m2m_dev, ctx,
...
static int mtk_mdp_m2m_release(struct file *file)
{
	...
	if (mdp->m2m.refcnt > 0) {
		...
		clear_bit(ST_M2M_OPEN, &mdp->state);
		mdp->m2m.refcnt = 0;
	}
	kfree(ctx);
...
int mtk_mdp_register_m2m_device(struct mtk_mdp_dev *mdp)
{
	...
	mdp->m2m.m2m_dev = v4l2_m2m_init(&mtk_mdp_m2m_ops);
	if (IS_ERR(mdp->m2m.m2m_dev)) {
		dev_err(dev, "failed to initialize v4l2-m2m device\n");
		ret = PTR_ERR(mdp->m2m.m2m_dev);
		goto err_m2m_init;
	}
...
err_vdev_register:
	v4l2_m2m_release(mdp->m2m.m2m_dev);
err_m2m_init:
