...
int mtk_mdp_enum_fmt_mplane(struct v4l2_fmtdesc *f)
{
	...
	fmt = mtk_mdp_find_fmt(NULL, NULL, f->index);
	if (!fmt)
		return -EINVAL;
...
int mtk_mdp_try_fmt_mplane(struct mtk_mdp_ctx *ctx, struct v4l2_format *f)
{
	...
	fmt = mtk_mdp_find_fmt(&pix_mp->pixelformat, NULL, 0);
	if (!fmt) {
...
int mtk_mdp_process_done(void *priv)
{
	...
	ctx = v4l2_m2m_get_curr_priv(mdp->m2m.m2m_dev);
	mutex_lock(&ctx->slock);

	if (test_and_clear_bit(ST_M2M_PEND, &mdp->state)) {
		if (test_and_clear_bit(ST_M2M_SUSPENDING, &mdp->state)) {
			set_bit(ST_M2M_SUSPENDED, &mdp->state);
			wake_up(&mdp->irq_queue);
			goto done_unlock;
		}

		mutex_unlock(&ctx->slock);
		mtk_mdp_m2m_job_finish(ctx, VB2_BUF_STATE_DONE);
