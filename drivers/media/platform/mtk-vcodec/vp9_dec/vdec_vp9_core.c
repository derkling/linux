...
static int vp9_setup_buf(struct vdec_vp9_inst *inst)
{
	...
	drv->seg_id_buf.va = (unsigned long)vpu_mapping_dm_addr(inst->dev,
		drv->seg_id_buf.vpua);
	if (drv->seg_id_buf.va == -EINVAL) {
		mtk_vcodec_err(inst, "drv->seg_id_buf.vpua=%x",
			drv->seg_id_buf.vpua);
		return -EINVAL;
	}
	...
	if (drv->seg_id_buf.pa == -EINVAL) {
		mtk_vcodec_err(inst, "drv->seg_id_buf.pa=%x",
			drv->seg_id_buf.vpua);
		return -EINVAL;
	}
	...
	drv->tile_buf.va = (unsigned long)vpu_mapping_dm_addr(inst->dev,
		drv->tile_buf.vpua);
	if (drv->tile_buf.va == -EINVAL) {
		mtk_vcodec_err(inst, "drv->tile_buf.vpua=%x",
			drv->tile_buf.vpua);
		return -EINVAL;
	}
	...
	if (drv->count_tbl_buf.va == -EINVAL) {
		mtk_vcodec_err(inst, "drv->count_tbl_buf.vpua=%x",
			drv->count_tbl_buf.vpua);
		return -EINVAL;
	}
	...
	if (drv->count_tbl_buf.pa == -EINVAL) {
		mtk_vcodec_err(inst, "drv->count_tbl_buf.vpua=%x",
			drv->count_tbl_buf.vpua);
		return -EINVAL;
	}
