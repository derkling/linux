...
static int hdlcd_load(struct drm_device *dev, unsigned long flags)
{
	...
	if (!res) {
		DRM_ERROR("failed to get memory resource\n");
		ret = -EINVAL;
		goto fail;
	}
	...
	if (!hdlcd->crtc.port) {
		/*
		 * It only makes sense to create the virtual connector if we
		 * don't have a physical way of controlling output
		 */
		DRM_INFO("no port provided, disabling physical connector\n");
		ret = hdlcd_create_virtual_connector(dev);
		if (ret < 0) {
			DRM_ERROR("failed to create virtual connector\n");
			goto fail;
		}
	} else {
		struct component_match *match = NULL;
		struct device_node *remote = of_graph_get_remote_port_parent(hdlcd->crtc.port);
		if (!remote || !of_device_is_available(remote)) {
			of_node_put(remote);
			DRM_ERROR("parent device of %s is not available\n",
				remote->full_name);
			ret = -ENXIO;
			goto fail;
		}
		component_match_add(dev->dev, &match, compare_dev, remote);
		of_node_put(remote);

		/* only physical connections need pxlclk */
		hdlcd->clk = clk_get(dev->dev, "pxlclk");
		if (IS_ERR(hdlcd->clk)) {
			DRM_ERROR("missing or invalid pxlclk\n");
