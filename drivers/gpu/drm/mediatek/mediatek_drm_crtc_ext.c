...
static void crtc_ext_power_on(struct crtc_ext_context *ctx)
{
	...
	ret = pm_runtime_get_sync(ctx->ddp_dev);
	if (ret < 0)
...
{
	pm_runtime_put_sync(ctx->ddp_dev);
...
static void crtc_ext_dpms(struct mtk_drm_crtc *crtc, int mode)
{
	...
	case DRM_MODE_DPMS_ON:
		crtc_ext_power_on(crtc->ctx);
		break;
	case DRM_MODE_DPMS_STANDBY:
	case DRM_MODE_DPMS_SUSPEND:
	case DRM_MODE_DPMS_OFF:
		crtc_ext_power_off(crtc->ctx);
		break;
	default:
...
static struct mediatek_drm_crtc_ops crtc_ext_crtc_ops = {
	...
	.ovl_layer_config = crtc_ext_ovl_layer_config,
...
static int crtc_ext_bind(struct device *dev, struct device *master, void *data)
{
	...
	DRM_INFO("DispClockOn ext\n");
	mediatek_ddp_clock_on(ctx->ddp_dev);
	crtc_ext_power_on(ctx);

	DRM_INFO("ExtDispPathSetup\n");
	ext_disp_path_setup(ctx->ddp_dev);

	DRM_INFO("ExtDispPathPowerOn\n");
	ext_disp_path_power_on(ctx->ovl1_regs, ctx->rdma1_regs,
		ctx->color1_regs);
...
static const struct of_device_id crtc_ext_driver_dt_match[] = {
	{ .compatible = "mediatek,mt8173-crtc-ext" },
	{},
};
