...
static void it6151_enable(struct drm_bridge *bridge)
{
	...
	regulator_set_voltage(ite_bridge->power_supplies,
		1800000, 1800000);
	err = regulator_enable(ite_bridge->power_supplies);
	if (err < 0) {
...
int it6151_bridge_attach(struct drm_bridge *bridge)
{
	...
	ite_bridge->connector.encoder = bridge->encoder;
...
{
	adjusted_mode->hdisplay = ALIGN(mode->hdisplay, 4);
...
static int it6151_suspend(struct device *dev)
{
	...
	if (!ite_bridge) {
		dev_err(&client->dev, "%s: ite_bridge is null\n", __func__);
		ret = -EINVAL;
		goto errcode;
	}


	it6151_disable(&ite_bridge->bridge);

	return 0;

errcode:
	dev_err(&client->dev, "%s: suspend failed\n", __func__);
	return ret;
}

static int it6151_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct it6151_bridge *ite_bridge = i2c_get_clientdata(client);
	int ret = 0;
	...
	it6151_enable(&ite_bridge->bridge);
...
static const struct dev_pm_ops it6151_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(it6151_suspend, it6151_resume)
};
#endif
