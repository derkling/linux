...
static void ps8640_pre_enable(struct drm_bridge *bridge)
{
	...
	regulator_set_voltage(ps_bridge->power_supplies, 3300000, 3300000);
	err = regulator_enable(ps_bridge->power_supplies);
	if (err < 0) {
...
static void ps8640_disable(struct drm_bridge *bridge)
{
	...
	regulator_disable(ps_bridge->power_supplies);
	gpiod_set_value(ps_bridge->gpio_rst_n, 0);
...
int ps8640_bridge_attach(struct drm_bridge *bridge)
{
	...
	ps_bridge->connector.dpms = DRM_MODE_DPMS_OFF;
	ps_bridge->connector.encoder = bridge->encoder;
...
static int ps8640_suspend(struct device *dev)
{
	...
	if (!ps_bridge) {
		dev_err(&client->dev, "%s: ps_bridge is null\n", __func__);
		ret = -EINVAL;
		goto errcode;
	}
...
static int ps8640_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct ps8640_bridge *ps_bridge = i2c_get_clientdata(client);
	int ret = 0;
	...
	ps8640_enable(&ps_bridge->bridge);
...
static const struct dev_pm_ops ps8640_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(ps8640_suspend, ps8640_resume)
};
#endif
