					   ...
					   int index, char *buf, int buf_len)
{
	...
	total_time = cmdq_core_get_time_in_ms(cqr->submit, cqr->done);
	acquire_thread_time = cmdq_core_get_time_in_ms(cqr->submit, cqr->trigger);
	begin_wait_time = cmdq_core_get_time_in_ms(cqr->submit, cqr->begin_wait);
#ifdef CMDQ_CORE_PROFILE_WAKE_UP
	wake_up_time = cmdq_core_get_time_in_ms(cqr->send_wake_up,
					   cqr->receive_wake_up);
#endif
	irq_time = cmdq_core_get_time_in_ms(cqr->trigger, cqr->got_irq);
	exec_time = cmdq_core_get_time_in_ms(cqr->trigger, cqr->waked_up);
				       ...
				       int tid)
{
	...
	value[2] = readl(gce_base_va + CMDQ_THR_WAIT_TOKEN_OFFSET +
			 CMDQ_THR_SHIFT * tid);
	value[3] = cmdq_thread_get_cookie(gce_base_va, tid);
						   ...
						   int index)
{
	...
	for (inner = 0;
		...
		do {
			...
			pc_va = cmdq_task_get_pc_and_inst(task, index, insts);
			if (pc_va) {
		       ...
		       const char *tag)
{
	...
	pc_va = cmdq_task_get_pc_and_inst(task, thread, insts);
	if (pc_va) {
...
void cmdq_core_dump_thread_pc(struct cmdq *cqctx, const int tid)
{
	int i;
	...
	for (i = 0; i < CMDQ_MAX_TASK_IN_THREAD; i++) {
		...
		if (cq_tsk) {
			pc_va = cmdq_task_get_pc_and_inst(cq_tsk, tid, insts);
			if (pc_va) {
				const u32 op =
...
void cmdq_core_dump_task_with_engine_flag(struct cmdq *cqctx,
					  u64 engine_flag)
{
	struct cmdq_task *cq_tsk;
	struct list_head *p;
...
void cmdq_core_dump_task_in_thread(struct cmdq *cqctx,
				   const int tid,
