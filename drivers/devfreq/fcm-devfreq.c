...
static unsigned long fcm_l3cache_calc_next_portions(struct fcm_devfreq *fcm)
{
	...
	if (up > 0) {
		...
		/* We can't go over the maximum possible value */
		if (portions > fcm->portion_max)
			portions = fcm->portion_max;
		return portions;
	...
	if (fcm->alg.usec_down >= (POLLING_DOWN_INTERVAL * fcm->polling_ms)) {
		...
		if (down > 0) {
			/* We can't go beneath minimum */
			if (fcm->cur_num_portions > down + fcm->portion_min)
				portions = fcm->cur_num_portions - down;
			else
				portions = fcm->portion_min;
		}
	}
