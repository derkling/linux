...
#ifdef l_ranges
static LIST_HEAD(new_list_head);
#else
static struct list_head *l_ranges;
#endif
...
static LIST_HEAD(__fsc_list);

/*The number of FSCs found*/
static u16 tot_fsc;

/*VARs for TESTING*/
static u32 test_comp;

static struct cpm_fsc *debug_fsc;
...
#define dprintk(msg...) pr_debug("cpm-gov-exh" msg)
#else
	...
	list_for_each_entry(p, &__fsc_list, node) {
		...
		for (i = 0; i < p->asms_count; i++) {
			...
			dprintk("id:%2hu t:%1u l:%3hu u:%3hu\n",
				r.id, r.range.type, r.range.lower,
				r.range.upper);
		}
	}
}
...
int __build_fsc_list_exhaustive(struct list_head *l_dev, u8 ndev)
{
	...
	for (idwr = 0; idwr < ndwr; idwr++) {
		...
		if (list_empty(l_ranges)) {
			/* no governor ranges are present yet:
			 * this is the first device level.
			 * DWR's ranges can be copied directly */
			/* nrgs = dwrs.asms_count; */
			dprintk("l_ranges empty: adding DWR ranges\n");
			for (irgs = 0; irgs < dwrs.asms_count; irgs++) {
				new_g_rg =
				    kzalloc(sizeof(struct cpm_gov_asm_range),
					    GFP_KERNEL);
				if (!new_g_rg) {
					eprintk("out-of-mem on"
							"cpm_gov_asm_range\n");
					return -ENOMEM;
				}
				...
				dprintk("adding range t:%1hu l:%3hu u:%3hu\n",
					new_g_rg->normal_range.range.type,
					new_g_rg->normal_range.range.lower,
			 ...
			 * nrgs = dwrs.asms_count */
			dprintk
			    ("l_ranges NOT empty: search if asm is  present\n");
			for (irgs = 0; irgs < dwrs.asms_count; irgs++) {
				...
				if (d_rgs.id == g_rgs->normal_range.id) {
					/* a candidate range for this asm
					 * exist in governor ranges. must
					 * be merged with the currend dwr
					 * range */
					dprintk("asm exist in local governor"
							"ranges\n");
					...
					dprintk("return merge:%2hu\n",
						merge_res);
					if (merge_res == -EINVAL) {
						/* no merging is possible: stop
						 * current dwr analysis */
						dprintk("no merge possible\n");
						break;
					} else {
						/* merging is possible: new
						 * candidate range for this asm
						 * must be added */
						...
						INIT_LIST_HEAD(&
							       (new_g_rg->old));
						new_g_rg->dev = dev;
						list_add_tail(&new_g_rg->old,
							      &g_rgs->old);
						list_replace(&g_rgs->node,
							     &new_g_rg->node);
						dprintk("adding range t:%1hu "
							"l:%3hu u:%3hu\n",
							new_g_rg->normal_range.
							range.type,
							new_g_rg->normal_range.
					...
					tot_grgs++;
					dprintk
					    ("adding range t:%1hu l:%3hu "
					     "u:%3hu\n",
					     new_g_rg->normal_range.range.type,
					     new_g_rg->normal_range.range.lower,
					     new_g_rg->normal_range.range.
					     upper);
					dprintk
					    ("ranges added total ranges %hu\n",
					     tot_grgs);
				}
			}
		}
		...
		if (merge_res != -EINVAL) {
			/*current DWR must be added to DWRs that map
			 * on current FSC */
			...
			if (ndev == tot_dev - 1) {
				/*last device has been query: new fsc can be
				 * added to fsc_list */
				...
				if (!new_fsc)
					return -ENOMEM;
				new_fsc->dwrs =
					kzalloc(sizeof(struct cpm_fsc_dwr) *
							tot_dev, GFP_KERNEL);
				if (!new_fsc->dwrs)
					return -ENOMEM;
				new_fsc->asms =
					kzalloc(sizeof(struct cpm_asm_range) *
							tot_grgs, GFP_KERNEL);
				new_fsc->id = tot_fsc++;
				new_fsc->gov_data = 0;
				new_fsc->pol_data = 0;
				...
				/* if this isn't the last device a new
				 * recursive call on the next device must
				 * be done */
				__build_fsc_list_exhaustive(l_dev->next,
							    ndev + 1);
			}
		}
		/* remove all modification to governor asms ranges made
		 * by current device-dwr */
...
int build_fsc_list_exhaustive(struct list_head *dev_list, u8 ndev)
{
	...
	curr_dwr = kzalloc(sizeof(struct cpm_fsc_dwr) * ndev,
					  GFP_KERNEL);
	if (!curr_dwr) {
...
#ifndef l_ranges
	l_ranges = kzalloc(sizeof(struct list_head), GFP_KERNEL);
	if (!l_ranges) {
