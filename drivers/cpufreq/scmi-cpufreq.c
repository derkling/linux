...
unsigned int __scmi_cpufreq_get_rate(struct cpufreq_policy *policy, bool poll)
{
	int ret;
	unsigned long rate;
	...
	ret = perf_ops->freq_get(priv->handle, priv->domain_id, &rate, poll);
	if (ret)
		return 0;
	return rate / 1000;
}
...
{
	return __scmi_cpufreq_get_rate(cpufreq_cpu_get_raw(cpu), false);
}
