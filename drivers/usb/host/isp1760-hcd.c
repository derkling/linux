				  ...
				  struct isp1760_qtd *qtd)
{
	...
	if (priv->atl_queued == 2)
		isp1760_writel(INTERRUPT_ENABLE_MASK, hcd->regs + INTERRUPT_ENABLE_SOT_MASK);
...
static void do_atl_int(struct usb_hcd *usb_hcd)
{
	...
	while (done_map) {
		...
		if (!nakcount && (dw3 & DW3_QTD_ACTIVE)) {
			...
			priv->atl_queued++;
			if (priv->atl_queued == 2)
				isp1760_writel(INTERRUPT_ENABLE_MASK, usb_hcd->regs + INTERRUPT_ENABLE_SOT_MASK);
	...
	if (priv->atl_queued <= 1)
		isp1760_writel(INTERRUPT_ENABLE_MASK, usb_hcd->regs + HC_INTERRUPT_ENABLE);
}
