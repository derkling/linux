...
static DEFINE_MUTEX(tzcpu_lock);
int tzcpu_register_done = 0;

void tscpu_thermal_zone_device_update(void)
{
	if(tzcpu_register_done)
		thermal_zone_device_update(thz_dev);
}
EXPORT_SYMBOL(tscpu_thermal_zone_device_update);
...
static int tscpu_register_thermal(void)
{
	...
	mutex_lock(&tzcpu_lock);
	tzcpu_register_done = 1;
	mutex_unlock(&tzcpu_lock);
