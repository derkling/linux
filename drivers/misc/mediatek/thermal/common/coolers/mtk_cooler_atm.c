...
#ifdef FAST_RESPONSE_ATM
/* [FIX ME] workaround to check tscpu temp to activate atm*/
static unsigned int cl_dev_adp_cpu_state_active_chk;
#endif
...
#ifdef FAST_RESPONSE_ATM
	/* [FIX ME] workaround to check tscpu temp to activate atm (tirp point: 58C)*/
	if ((cl_dev_adp_cpu_state_active_chk == 1) && (curr_temp > 58000)) {
		tscpu_thermal_zone_device_update();
		tscpu_printk("tscpu_thermal_zone_device_update! Tp %ld, Tc %ld, cl_state: %d\n",
			prev_temp, curr_temp, cl_dev_adp_cpu_state_active);
	}
#endif
...
static enum hrtimer_restart atm_loop(struct hrtimer *timer)
{
	...
	/* [FIX ME] workaround to check tscpu temp to activate atm (exit point: 52C)*/
	if ((cl_dev_adp_cpu_state_active == 0) && (atm_curr_maxtj < 52000))
		cl_dev_adp_cpu_state_active_chk = 1;
	else if (cl_dev_adp_cpu_state_active == 1)
		cl_dev_adp_cpu_state_active_chk = 0;
