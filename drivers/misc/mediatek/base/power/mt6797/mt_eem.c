...
/* BIG */
unsigned int bigFreq_FY_7[NR_FREQ] = {
	2314000, 2119000, 1989000, 1781000, 1677000,
	1495000, 1248000, 1001000, 845000,   676000
};

/* GPU */
unsigned int gpuFreq_FY_7[NR_FREQ] = {
	780000, 700000, 610000, 520000, 442500,
	365000, 238000, 238000, 154500, 154500};

/* L */
unsigned int L_Freq_FY_7[NR_FREQ] = {
	1625000, 1495000, 1417000, 1209000, 1092000,
	 949000,  832000,  650000,  468000,  325000
};

/* LL */
unsigned int LL_Freq_FY_7[NR_FREQ] = {
	1287000, 1118000, 949000, 897000, 806000,
	715000,  559000,  416000, 338000, 221000
};

/* CCI */
unsigned int cciFreq_FY_7[NR_FREQ] = {
	923000, 858000, 754000, 637000, 611000,
	533000, 416000, 377000, 286000, 169000};
...
/* FAN53555 */
unsigned int gpuFy_7[10] = {
	0x29, 0x26, 0x22, 0x1F, 0x1B,
	0x18, 0x14, 0x14, 0x10, 0x10
};
...
unsigned int gpuOutput[NR_FREQ];
...
static unsigned int ITurbo_offset[NR_FREQ];
...
static void restore_record(struct eem_det *det)
{
	...
	for (i = 0; i < NR_FREQ; i++) {
		...
		if (det_to_id(det) == EEM_DET_2L)
			...
			vWrite = (vWrite & (~0x3FFF)) |
				(((*(recordTbl + (i + 10) * 8 + 6) & 0x7F) << 7) |
				(*(recordTbl + (i + 10) * 8 + 7) & 0x7F));
		else if (det_to_id(det) == EEM_DET_CCI)
			vWrite = (vWrite & (~0x3FFF)) |
				(((*(recordTbl + (i + 20) * 8 + 6) & 0x7F) << 7) |
				(*(recordTbl + (i + 20) * 8 + 7) & 0x7F));
		else if (det_to_id(det) == EEM_DET_BIG)
			vWrite = (vWrite & (~0x3FFF)) |
				(((*(recordTbl + (i + 30) * 8 + 6) & 0x7F) << 7) |
				(*(recordTbl + (i + 30) * 8 + 7) & 0x7F));
		det->recordRef[i*2] = vWrite;
	}
	det->recordRef[NR_FREQ * 2] = 0xFFFFFFFF;
	mb(); /* SRAM writing */
...
static int set_volt_gpu(struct eem_det *det)
{
	...
	*/
	for (i = 0; i < NR_FREQ; i++)
		gpuOutput[i] = det->volt_tbl_pmic[i];

	#ifdef EARLY_PORTING
		return 0;
	#else
		return mt_gpufreq_update_volt(gpuOutput, NR_FREQ);
	#endif
}
	...
	for (i = 0; i < det->num_freq_tbl; i++) {
		...
		case EEM_CTRL_BIG:
			...
			(unsigned int)(clamp(
					...
					det->VMIN, det->VMAX)),
			(unsigned int)(*(recordTbl + ((i + 30) * 8) + 7) & 0x7F)
			);
			break;
		...
		case EEM_CTRL_L:
			...
			*/
			...
			(unsigned int)(clamp(
				...
				det->ops->eem_2_pmic(det, det->VMIN),
				det->ops->eem_2_pmic(det, det->VMAX))),
			(unsigned int)(*(recordTbl + ((i + 10) * 8) + 7) & 0x7F)
			);
			/*
			if (eem_log_en)
		...
		case EEM_CTRL_CCI:
			...
			(unsigned int)(clamp(
				...
				det->ops->eem_2_pmic(det, det->VMIN),
				det->ops->eem_2_pmic(det, det->VMAX))),
			(unsigned int)(*(recordTbl + ((i + 20) * 8) + 7) & 0x7F)
			);
			break;
		...
		case CPU_DOWN_PREPARE:
			if ((1 == ITurboRun) && (ITURBO_CPU_NUM == big_cpus) && (MT_EEM_CPU_B == cluster_id)) {
				...
				det->recordRef[0] = ((det->recordRef[0] & 0x3FFF) |
					((*(recordTbl + (30 * 8) + 5) & 0x3FFFF) << 14));

				det->recordRef[1] =
					((*(recordTbl + (30 * 8) + 0) & 0x1F) << 21) |
					((*(recordTbl + (30 * 8) + 1) & 0x1FF) << 12) |
					((*(recordTbl + (30 * 8) + 2) & 0x1F) << 7) |
					((*(recordTbl + (30 * 8) + 3) & 0x7) << 4) |
					(*(recordTbl +  (30 * 8) + 4) & 0xF);
				mb(); /* SRAM writing */
				mt_ptp_unlock(&flags);
			} else {
				if (eem_log_en)
		...
		case CPU_UP_PREPARE:
			if ((0 == ITurboRun) && (8 == cpu) && (0 == big_cpus)) {
				...
				eem_error("The freq percentage is : (%d)\n",
					(*(recordTbl + (30 * 8) + 5) & 0x3FFFF) * 100 / 2048);

				swReq = 0;
				while ((mt_secure_call(0x8200035F, 0x10222498, 0, 0) >> 1) >
					(*(recordTbl + (30 * 8) + 5) & 0x3FFFF)) {
					udelay(120);
					if (eem_log_en)
						eem_error("SWREQ = %x",
					...
					if (swReq >= 100) {
						eem_get_semaphore();
						BigIDVFSFreq((*(recordTbl + (30 * 8) + 5) & 0x3FFFF) * 100 / 2048);
				...
				if (((mt_secure_call(0x8200035F, 0x10222498, 0, 0) >> 1) >
					(*(recordTbl + (30 * 8) + 5) & 0x3FFFF)) && (swReq >= 100))
							BUG_ON(swReq);
...
static int __init eem_conf(void)
{
	...
	for (i = 0; i < NR_FREQ; i++) {
		...
		*/
		recordRef[i * 2 + 24] =
			((*(recordTbl + ((i + 10) * 8) + 5) & 0x3FFFF) << 14) |
			((*(recordTbl + ((i + 10) * 8) + 6) & 0x7F) << 7) |
			((*(recordTbl + ((i + 10) * 8) + 7) & 0x7F));

		/* [25:21] = dcmdiv, [20:12] = DDS, [11:7] = clkdiv, [6:4] = postdiv, [3:0] = CFindex */
		recordRef[i * 2 + 24 + 1] =
			((*(recordTbl + ((i + 10) * 8) + 0) & 0x1F) << 21) |
			((*(recordTbl + ((i + 10) * 8) + 1) & 0x1FF) << 12) |
			((*(recordTbl + ((i + 10) * 8) + 2) & 0x1F) << 7) |
			((*(recordTbl + ((i + 10) * 8) + 3) & 0x7) << 4) |
			((*(recordTbl + ((i + 10) * 8) + 4) & 0xF));

		/* CCI
		[31:14] = iDVFS % (/2), [13:7] = Vsram pmic value, [6:0] = Vproc pmic value
		*/
		recordRef[i * 2 + 48] =
			((*(recordTbl + ((i + 20) * 8) + 5) & 0x3FFFF) << 14) |
			((*(recordTbl + ((i + 20) * 8) + 6) & 0x7F) << 7) |
			((*(recordTbl + ((i + 20) * 8) + 7) & 0x7F));

		/* [25:21] = dcmdiv, [20:12] = DDS, [11:7] = clkdiv, [6:4] = postdiv, [3:0] = CFindex */
		recordRef[i * 2 + 48 + 1] =
			((*(recordTbl + ((i + 20) * 8) + 0) & 0x1F) << 21) |
			((*(recordTbl + ((i + 20) * 8) + 1) & 0x1FF) << 12) |
			((*(recordTbl + ((i + 20) * 8) + 2) & 0x1F) << 7) |
			((*(recordTbl + ((i + 20) * 8) + 3) & 0x7) << 4) |
			((*(recordTbl + ((i + 20) * 8) + 4) & 0xF));

		/* BIG
		[31:14] = iDVFS % (/2), [13:7] = Vsram pmic value, [6:0] = Vproc pmic value
		*/
		recordRef[i * 2 + 72] =
			((*(recordTbl + ((i + 30) * 8) + 5) & 0x3FFFF) << 14) |
			((*(recordTbl + ((i + 30) * 8) + 6) & 0x7F) << 7) |
			((*(recordTbl + ((i + 30) * 8) + 7) & 0x7F));

		/* [25:21] = dcmdiv, [20:12] = DDS, [11:7] = clkdiv, [6:4] = postdiv, [3:0] = CFindex */
		recordRef[i * 2 + 72 + 1] =
			((*(recordTbl + ((i + 30) * 8) + 0) & 0x1F) << 21) |
			((*(recordTbl + ((i + 30) * 8) + 1) & 0x1FF) << 12) |
			((*(recordTbl + ((i + 30) * 8) + 2) & 0x1F) << 7) |
			((*(recordTbl + ((i + 30) * 8) + 3) & 0x7) << 4) |
			((*(recordTbl + ((i + 30) * 8) + 4) & 0xF));
	}
	recordRef[i*2] = 0xffffffff;
	recordRef[i*2+24] = 0xffffffff;
	recordRef[i*2+48] = 0xffffffff;
	...
	for (i = 0; i < NR_FREQ; i++)
		eem_debug("L (iDVFS, 0x%x), (Vs, 0x%x), (Vp, 0x%x), (F_Setting)(%x, %x, %x, %x, %x)\n",
			((*(recordRef + 24 + (i * 2))) >> 14) & 0x3FFFF,
			((*(recordRef + 24 + (i * 2))) >> 7) & 0x7F,
			(*(recordRef + 24 + (i * 2))) & 0x7F,
			((*(recordRef + 24 + (i * 2) + 1)) >> 21) & 0x1F,
			((*(recordRef + 24 + (i * 2) + 1)) >> 12) & 0x1FF,
			((*(recordRef + 24 + (i * 2) + 1)) >> 7) & 0x1F,
			((*(recordRef + 24 + (i * 2) + 1)) >> 4) & 0x7,
			((*(recordRef + 24 + (i * 2) + 1)) & 0xF)
			);

	for (i = 0; i < NR_FREQ; i++)
		eem_debug("CCI (iDVFS, 0x%x), (Vs, 0x%x), (Vp, 0x%x), (F_Setting)(%x, %x, %x, %x, %x)\n",
			((*(recordRef + 48 + (i * 2))) >> 14) & 0x3FFFF,
			((*(recordRef + 48 + (i * 2))) >> 7) & 0x7F,
			(*(recordRef + 48 + (i * 2))) & 0x7F,
			((*(recordRef + 48 + (i * 2) + 1)) >> 21) & 0x1F,
			((*(recordRef + 48 + (i * 2) + 1)) >> 12) & 0x1FF,
			((*(recordRef + 48 + (i * 2) + 1)) >> 7) & 0x1F,
			((*(recordRef + 48 + (i * 2) + 1)) >> 4) & 0x7,
			((*(recordRef + 48 + (i * 2) + 1)) & 0xF)
			);
