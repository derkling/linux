				 ...
				 char *buf)
{
	...
	return sprintf(buf, "%lu\n",
			drv_scale_cpu_capacity(NULL, cpu->dev.id));
}
				  ...
				  size_t count)
{
	...
	if (count) {
		...
		for_each_cpu(i, &cpu_topology[this_cpu].core_sibling)
			drv_set_capacity_scale(i, new_capacity);
...
void drv_normalize_cpu_capacity(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		capacity = (raw_capacity[cpu] << SCHED_CAPACITY_SHIFT)
			/ capacity_scale;
		drv_set_capacity_scale(cpu, capacity);
		pr_debug("cpu_capacity: CPU%d cpu_capacity=%lu\n",
			cpu, drv_scale_cpu_capacity(NULL, cpu));
	}
	mutex_unlock(&cpu_scale_mutex);
}
			   ...
			   void *data)
{
	...
	case CPUFREQ_NOTIFY:
		...
		for_each_cpu(cpu, policy->related_cpus) {
			raw_capacity[cpu] = drv_scale_cpu_capacity(NULL, cpu) *
					    policy->cpuinfo.max_freq / 1000UL;
			capacity_scale = max(raw_capacity[cpu], capacity_scale);
		}
		if (cpumask_empty(cpus_to_visit)) {
			drv_normalize_cpu_capacity();
			kfree(raw_capacity);
