...
static int mt8173_rt5650_rt5676_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	ret = snd_soc_card_jack_new(card, "Headphone Jack", SND_JACK_HEADPHONE,
			       &mt8173_rt5650_rt5676_hp_jack, NULL, 0);
	if (ret) {
		dev_err(card->dev, "Can't new HP Jack %d\n", ret);
		return ret;
	}
	ret = snd_soc_card_jack_new(card, "Mic Jack", SND_JACK_MICROPHONE,
			       &mt8173_rt5650_rt5676_mic_jack, NULL, 0);
	if (ret) {
	...
	return rt5645_set_jack_detect(codec, &mt8173_rt5650_rt5676_hp_jack,
				      &mt8173_rt5650_rt5676_mic_jack, NULL);
}
